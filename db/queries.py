...
def get_weight_on_date(target_date: date) -> dict | None:
    """
    ...
    try:
        ...
        cursor.execute("SELECT * FROM weights WHERE date = %s",                # %s es el placeholder — psycopg2 lo sustituye por target_date de forma segura (evita SQL injection), la coma después de target_date es necesaria para que sea una tupla
                   (target_date,))
...
def get_last_weight() -> dict | None:
    ...
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute("SELECT * FROM weights ORDER BY date DESC LIMIT 1")
...
def insert_weight(new_weight: float) -> str:
    """
    ...
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute("INSERT INTO weights (date, weight) VALUES (%s, %s)",
                       (datetime.now().date(), new_weight))
...
def update_weight(new_weight: float) -> str:
    """
    ...
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute("UPDATE weights SET weight = %s WHERE date = %s",
                       (new_weight, datetime.now().date()))
        conn.commit()
        return "updated"
    except Exception as e:
        conn.rollback()
...
def get_active_phase() -> dict | None:
    ...
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute("SELECT * FROM phases WHERE end_date IS NULL")
        return cursor.fetchone()
    except Exception as e:
        raise e
    finally:
...
def close_phase(end_date: date) -> str:
    """
    ...
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute("UPDATE phases SET end_date = %s WHERE end_date IS NULL",
                       (end_date,))
        conn.commit()
...
def insert_phase(new_phase: dict) -> str:
    """
    ...
    try:
        ...
        cursor.execute("INSERT INTO phases (start_date, phase_type, weight_goal, date_goal) "
                       "VALUES (%s, %s, %s, %s)",
                       (datetime.now().date(), new_phase["phase_type"],
                        new_phase.get("weight_goal"), date_goal))
        conn.commit()
        return "added"
    except Exception as e:
        conn.rollback()
